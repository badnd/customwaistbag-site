...
import { productCategories, products, productsForCategory } from '../data/catalogData';
import { ProductCard } from '../components/CatalogPages';
...
export default function HomePage() {
  return <>
    ...
    <section className="section home-catalog"><div className="shell"><div className="section-head"><div className="eyebrow">Product categories</div><h2>Choose the waist bag structure that fits your use</h2><p>Eight current models are organized by carry style, with complete specifications and inquiry-only pricing on each product page.</p></div><div className="home-category-grid">{featuredCategories.map((category) => <article className="home-category-card" key={category.slug}><Link className="home-category-media" href={`/${category.slug}`}><img src={productCardImage(category.representative)} alt={`${category.title} representative model ${category.representative.model}`} loading="lazy" width="1122" height="1402" /></Link><div className="home-category-copy"><p className="product-model">{category.representative.model}</p><h3><Link href={`/${category.slug}`}>{category.title}</Link></h3><p>{category.description}</p><Link className="button secondary" href={`/${category.slug}`}>Browse Category</Link></div></article>)}</div></div></section>
    <section className="section alt home-featured-products"><div className="shell"><div className="section-head"><div className="eyebrow">Featured products</div><h2>Current waist bag models, newest first</h2><p>Newly published models enter this wall automatically while every complete product page and gallery remains available.</p></div><div className="product-card-grid">{featuredProducts.map((product) => <ProductCard key={product.model} product={product} locale="en" />)}</div><p className="moq-note">* Final MOQ depends on style, fabric and logo method. Anna will confirm in your quotation.</p></div></section>

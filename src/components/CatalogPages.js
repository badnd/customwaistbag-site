...
export function ProductPage({ product, locale = 'en' }) {
  ...
  return (
    <>
      <JsonLd data={schema} />
      <section className="product-intro"><div className="shell product-intro-grid"><div><div className="eyebrow">{product.model}</div><h1>{ru ? product.ruTitle : product.title}</h1><p className="product-tagline">{ru ? product.ruTagline : product.tagline}</p><p className="lead">{ru ? product.ruIntro : product.intro}</p><div className="actions"><a className="button" href="#quote">{ru ? 'Запросить расчет' : 'Request a Quote'}</a></div></div><img src={product.gallery[0]} alt={`${product.model} ${ru ? product.ruType : product.type} - ${text(product.subjects[0], locale)}`} width="1122" height="1402" /></div></section>

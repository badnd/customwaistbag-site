...
import { ProductCard } from './CatalogPages';
...
export function RuHomePage() {
  return <>
    ...
    <section className="section home-catalog"><div className="shell"><div className="section-head"><div className="eyebrow">Категории продукции</div><h2>Выберите конструкцию поясной сумки под вашу задачу</h2><p>Восемь актуальных моделей распределены по способу ношения. На каждой странице доступны характеристики и расчет только по запросу.</p></div><div className="home-category-grid">{featuredCategories.map((category) => <article className="home-category-card" key={category.slug}><Link className="home-category-media" href={`/ru/${category.slug}`}><img src={productCardImage(category.representative)} alt={`${category.ruTitle}, модель ${category.representative.model}`} loading="lazy" width="1122" height="1402" /></Link><div className="home-category-copy"><p className="product-model">{category.representative.model}</p><h3><Link href={`/ru/${category.slug}`}>{category.ruTitle}</Link></h3><p>{category.ruDescription}</p><Link className="button secondary" href={`/ru/${category.slug}`}>Открыть категорию</Link></div></article>)}</div></div></section>
    <section className="section alt home-featured-products"><div className="shell"><div className="section-head"><div className="eyebrow">Избранные модели</div><h2>Актуальные поясные сумки: сначала новые</h2><p>Новые модели автоматически появляются в этой витрине, а полные страницы товаров и галереи остаются доступными.</p></div><div className="product-card-grid">{featuredProducts.map((product) => <ProductCard key={product.model} product={product} locale="ru" />)}</div><p className="moq-note">* Итоговый MOQ зависит от модели, материала и способа нанесения логотипа. Анна подтвердит его в предложении.</p></div></section>

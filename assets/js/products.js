...
(async function(){
 ...
 function assetUrl(path){
   ...
   const base=(window.SITE_CONFIG?.assetBase||'').replace(/\/$/,'');
   return base&&/^assets\//.test(path)?base+'/'+path:prefix+path;
 }
 if(filters){
 ...
 function render(filter='all'){
   ...
   if(grid)grid.innerHTML=list.map(p=>`<article class="card product-card"><a href="${prefix}products/${p.slug}.html"><img src="${assetUrl(p.mainImage)}" alt="${p.title?.en||p.title||p.model}" loading="lazy"><div class="card-pad"><span class="badge">${p.model||'Custom'}</span><h3>${p.title?.en||p.title}</h3><p>${p.shortDescription?.en||p.shortDescription||''}</p></div></a></article>`).join('');
